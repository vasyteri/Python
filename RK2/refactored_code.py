...
def query1_browsers_with_ov_ending(browsers, computers):
    """
    Запрос 1: Список всех браузеров, у которых название заканчивается на «ов»,
...
def query2_computers_avg_memory(browsers, computers):
    """
    Запрос 2: Список компьютеров со средней нагрузкой памяти браузеров
...
def main():
    ...
    for comp_model, avg_memory in result2:
        print(f"Компьютер '{comp_model}': средняя нагрузка памяти = {avg_memory:.2f} МБ")
    
    print("\n" + "=" * 80)
    print("ЗАПРОС №3")
    print("=" * 80)
    result3 = query3_computers_starting_with_a(browsers, computers, browser_computers)
    for computer, computer_browsers in result3:
